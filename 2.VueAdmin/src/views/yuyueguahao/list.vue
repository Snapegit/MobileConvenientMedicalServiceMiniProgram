	...
	<div>
		<div class="app-contain">
			<div class="list_search_view">
				<el-form :model="searchQuery" class="search_form" >
					<div class="search_view">
						...
						<div class="search_box">
							<el-input class="search_inp" v-model="searchQuery.yishengxingming" placeholder="医生姓名"
								clearable>
							</el-input>
						</div>
					</div>
					<div class="search_view">
						...
						<div class="search_box">
							<el-select
								class="search_sel"
								clearable
								v-model="searchQuery.keshi" 
								placeholder="科室"
								>
								<el-option v-for="item in keshiLists" :label="item" :value="item"></el-option>
							</el-select>
						</div>
					</div>
				...
				<br>
				<div class="btn_view">
					<el-button type="success" @click="addClick" v-if="btnAuth('yuyueguahao','新增')">新增</el-button>
					<el-button  v-if=" btnAuth('yuyueguahao','查看')" type="info"  :disabled="selRows.length==1?false:true" @click="infoClick(null)">详情</el-button>
					<el-button type="primary" :disabled="selRows.length==1?false:true" @click="editClick" v-if=" btnAuth('yuyueguahao','修改')">修改</el-button>
					<el-button type="danger" :disabled="selRows.length?false:true" @click="delClick(null)"  v-if="btnAuth('yuyueguahao','删除')">删除</el-button>
				</div>
			</div>
			<br>
			<el-table
				...
				<el-table-column
					...
					label="预约须知">
					<template #default="scope">
						{{scope.row.yuyuexuzhi}}
					</template>
				</el-table-column>
				<el-table-column label="操作" width="300" :resizable='true' :sortable='true' align="left" header-align="center">
					<template #default="scope">
						<el-button type="info" v-if=" btnAuth('yuyueguahao','查看')" @click="infoClick(scope.row.id)">详情</el-button>
						<el-button v-if="btnAuth('yuyueguahao','挂号')" type="success" @click="guahaoCrossAddOrUpdateHandler(scope.row,'cross','','','')">挂号</el-button>
					</template>
				</el-table-column>
			</el-table>
			<el-pagination 
		...
		<guahaoFormModel ref="guahaoFormModelRef" @formModelChange="formModelChange"></guahaoFormModel>
	</div>
</template>
<script setup>
	...
	const getList = () => {
		...
		params['sort'] = 'id'
		params['order'] = 'desc'
		if(searchQuery.value.yishengxingming&&searchQuery.value.yishengxingming!=''){
			params['yishengxingming'] = '%' + searchQuery.value.yishengxingming + '%'
		}
	...
	import guahaoFormModel from '@/views/guahao/formModel'
	const guahaoFormModelRef = ref(null)
    const guahaoCrossAddOrUpdateHandler = (row,type,crossOptAudit,statusColumnName,tips,statusColumnValue) => {
		...
		nextTick(()=>{
			guahaoFormModelRef.value.init(row.id,'cross','挂号',row,'yuyueguahao',statusColumnName,tips,statusColumnValue)
		})
    }
	//初始化
	const init = () => {
