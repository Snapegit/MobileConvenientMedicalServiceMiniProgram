...
<script>
	export default {
		...
		async onShow(options) {
			...
			let cleanType = uni.getStorageSync('discussdiscussyiliaozhishiCleanType')
			if(cleanType){
				uni.removeStorageSync('discussdiscussyiliaozhishiCleanType')
				this.mescroll.num = 1
            ...
            if(crossCleanType) {
                ...
                res = await this.$api.info('discussyiliaozhishi', this.id);
		...
		methods: {
			// 支付
			onPayTap(){
				uni.setStorageSync('paytable','discussyiliaozhishi');
			...
			onAcrossTap(tableName,crossOptAudit,statusColumnName,tips,statusColumnValue){
				...
				uni.setStorageSync('crossTable','discussyiliaozhishi');
			...
			async init(){
				...
				let res = await this.$api.info('discussyiliaozhishi', this.id);
				this.detail = res.data;
			},
			...
			async upCallback(mescroll) {
                let res = await this.$api.list('discussdiscussyiliaozhishi', {
					page: mescroll.num,
					limit: 10,
					refid: this.id
				});
			...
			async onCommentTap() {
				...
				this.$utils.jump(`../discussdiscussyiliaozhishi/add-or-update?refid=${this.id}`)
			},
			onSHTap() {
				if(!this.user){
