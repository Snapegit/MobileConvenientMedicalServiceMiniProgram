	...
	export default {
		...
		async onLoad(options) {
			...
			if (options.id) {
				...
				// 获取信息
				res = await this.$api.info(`tijianfenlei`, this.ruleForm.id);
...
//跨表计算判断
				var obj;
				if((!this.ruleForm.tijianfenlei)){
					this.$utils.msg(`体检分类不能为空`);
					return
				}
				//更新跨表属性
			       var crossuserid;
				...
				if(crossrefid && crossuserid) {
					...
					let res = await this.$api.list(`tijianfenlei`, params);
                ...
                //跨表计算
						if(this.ruleForm.id){
							await this.$api.update(`tijianfenlei`, this.ruleForm);
						}else{
							await this.$api.add(`tijianfenlei`, this.ruleForm);
						}
